...
var reserved = 0x00  //eslint-disable-line
...
function discover (tracker, infoHash, peerId) { //eslint-disable-line
  ...
  discover.on('warning', function (warning) {
    console.log('Warning thrown on discovery', warning)
  })
  discover.on('error', function (discoveryError) {
    console.log('Error emitted from torrent discovery ', discoveryError)
  })
...
function connectToTracker (tracker, infoHash, peerId) {
  ...
  sock.on('message', function (msg) {
    ...
    if (msgAction === action.CONNECT) {
      ...
      var connectionIdUpper = msg.readUInt32BE(8)
      var connectionIdLower = msg.readUInt32BE(12)
      console.log('Action: ' + msgAction + ' transId ' + transId + ' conn_ID ' + connectionIdUpper + '' + connectionIdLower)
      const connectionId = Buffer.concat([toInt32(connectionIdUpper), toInt32(connectionIdLower)])
